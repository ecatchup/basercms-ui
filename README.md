...
ルート要素（`.bca-search-select`）の既定は `display: inline-block` で、幅は**最も長い選択肢**に合わせた内容幅になります（ネイティブの `<select>` と同じ挙動）。部品が親要素いっぱいに広がることはありません。トリガーとドロップダウンはどちらもルート要素の幅に追従する為、利用側が指定するのはルート要素の幅だけで済みます。

選択を変えても幅は変わらず、ドロップダウンの項目も折り返しません。これは JS で幅を計算しているのではなく、高さ 0 の実測用要素（`.bca-search-select__sizer`）をトリガー内に描画してブラウザに測らせている為で、フォントや文字サイズが変わっても追従します。実測用要素の文字列はテキストノードではなく `data-text` 属性で持ち、CSS の `content: attr(data-text)` で描画される為、`getByText` 等の DOM 検索に二重にヒットすることはありません。

この挙動が不要な場合は `sizeToLongestOption={false}` を渡してください（幅は選択中のラベルの長さに応じて変わります）。
